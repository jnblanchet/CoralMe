...
if dd==1 
	...
	out(1:bw,1:bw)=filler.*in(1,1);
	out(bw+hh+1:2*bw+hh,1:bw)=filler.*in(hh,1);
	out(1:bw,bw+1+ww:2*bw+ww)=filler.*in(1,ww);
	out(bw+hh+1:2*bw+hh,bw+1+ww:2*bw+ww)=filler.*in(hh,ww);
    ...
    for ii=1:dd
    	out(1:bw,1:bw,ii)=filler.*in(1,1,ii);
		out(bw+hh+1:2*bw+hh,1:bw,ii)=filler.*in(hh,1,ii);
		out(1:bw,bw+1+ww:2*bw+ww,ii)=filler.*in(1,ww,ii);
		out(bw+hh+1:2*bw+hh,bw+1+ww:2*bw+ww,ii)=filler.*in(hh,ww,ii);
